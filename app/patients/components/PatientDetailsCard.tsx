    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                ...
                <div><label className="text-xs font-bold text-gray-400 uppercase">Diagnosis</label>{isEditingDetails ? <textarea className="w-full border p-1 text-sm rounded" rows={2} value={patient.history || ""} onChange={e => setPatient({ ...patient, history: e.target.value })} /> : <p className="text-sm italic text-gray-600">{patient.history || "-"}</p>}</div>
                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-dashed">
                    <div><label className="text-[10px] font-bold text-gray-400 uppercase flex items-center gap-1"><Scale size={10} /> Weight</label>{isEditingDetails ? <input placeholder="e.g. 70 kg" className="w-full border-b text-sm" value={patient.initialWeight || ""} onChange={e => setPatient({ ...patient, initialWeight: e.target.value })} /> : <p className="text-sm font-bold text-gray-700">{patient.initialWeight || "-"}</p>}</div>
                    <div><label className="text-[10px] font-bold text-gray-400 uppercase flex items-center gap-1">Blood Pressure</label>{isEditingDetails ? <input placeholder="e.g. 120/80" className="w-full border-b text-sm" value={patient.currentWeight || ""} onChange={e => setPatient({ ...patient, currentWeight: e.target.value })} /> : <p className="text-sm font-bold text-red-700">{patient.currentWeight || "-"}</p>}</div>
                </div>
                <div className="pt-2">
