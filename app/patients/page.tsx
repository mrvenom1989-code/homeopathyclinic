    ...
    <div className="min-h-screen bg-[#FDFBF7] font-sans text-neutral-800">
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-serif font-bold text-[#0f172a] flex items-center gap-3">
              <Users className="text-[#0284c7]" /> Patient Management
            </h1>
          ...
          <button
            ...
            className="bg-[#0f172a] text-white px-6 py-2.5 rounded-lg font-bold shadow-lg hover:bg-[#020617] transition flex items-center gap-2"
          >
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 p-2.5 border rounded-lg text-sm focus:outline-none focus:border-[#0284c7]"
              value={search}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  <tr><td colSpan={7} className="p-8 text-center"><Loader2 className="animate-spin inline text-[#0284c7]" /></td></tr>
                ) : patients.length === 0 ? (
                  ...
                  patients.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-gray-50 group transition">
                        <td className="p-4 font-mono font-bold text-[#0284c7]">{p.readableId || "N/A"}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in duration-200">
            <div className="bg-[#0f172a] p-4 flex justify-between items-center text-white shrink-0">
              ...
              <button onClick={() => setIsModalOpen(false)} className="hover:text-[#0284c7]"><X size={20} /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input required className="w-full p-2 border rounded focus:border-[#0284c7] outline-none" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Phone *</label>
                  <input required className="w-full p-2 border rounded focus:border-[#0284c7] outline-none" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Age</label>
                    <input type="number" className="w-full p-2 border rounded focus:border-[#0284c7] outline-none" value={formData.age} onChange={e => setFormData({ ...formData, age: e.target.value })} />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input placeholder="e.g. 5'10&quot; or 175cm" className="w-full p-2 border rounded focus:border-[#0284c7] outline-none" value={formData.height} onChange={e => setFormData({ ...formData, height: e.target.value })} />
                </div>
              ...
              <div className="border-t pt-4">
                <button
                  ...
                  className="flex items-center gap-2 text-sm font-bold text-[#0284c7] hover:underline w-full justify-center"
                >
              ...
              <div className="pt-4 border-t flex justify-end gap-3 shrink-0 bg-white sticky bottom-0">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="px-6 py-2 bg-[#0f172a] text-white rounded-lg text-sm font-bold hover:bg-[#020617] flex items-center gap-2 disabled:opacity-50"
                >
      ...
      {isWalletModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 animate-in zoom-in-95">
            <h3 className="text-lg font-bold text-[#0f172a] mb-4 flex items-center gap-2"><Wallet /> Update Wallet</h3>
